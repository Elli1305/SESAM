<template>
  <q-page class="column justify-evenly" style="padding: 2em 5em">
    ...
    <div class="row self-center">
      <q-table :columns="columns" :filter="search" :rows="rows" :rows-per-page-options="[0]"
               :separator="'cell'" row-key="name" style="width: 80vw; height: 50vh">
        ...
        <template v-slot:top-right>
          <q-btn :label="t('admin.credentialAdministration.new')" borderless color="grey-6" flat icon="add" rounded
                 to="/add_credential"/>
          <q-input v-model="search" :placeholder="t('common.search')" borderless class="q-ml-lg" debounce="300" dense>
        ...
        <template v-slot:body-cell-actions="props">
          <q-td :props="props">
            <q-btn :to="`/credential_administration/${'issuer' in props.row ? 'internal' : 'external'}/${props.row.id}`"
                   color="grey" dense flat icon="edit" round></q-btn>
          </q-td>
        </template>
      </q-table>
    </div>
