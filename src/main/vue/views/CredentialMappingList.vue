<template>
    ...
    <div class="row self-center">
      <q-table
        ...
        <template v-slot:top-right>
          <q-btn class="q-mr-xs" flat color="grey" :label="t('admin.credentialMapping.newCategory')" icon="add" rounded
                 @click="prompt=true"/>
          <q-input class="q-ml-xs" outlined rounded dense debounce="250" v-model="filter"
        ...
        <template v-slot:body-cell-actions="props">
          <q-td :props="props">
            <q-btn dense round flat color="grey" @click="changeCategory = true; openForm(props.row)"
                   icon="edit"></q-btn>
  ...
  <q-dialog v-model="prompt" persistent>
    <q-card style="background-color: var(--bg-color); color: var(--text-color)">
      ...
      <q-card-section class="q-pt-none">
        <q-input dense :label="t('admin.credentialMapping.categoryName')" v-model="address" autofocus
                 @keyup.enter="prompt = false" :placeholder="t('common.category')">
          <template v-slot:append>
            <q-icon name="edit"/>
          </template>
        </q-input>
  ...
  <q-dialog v-model="changeCategory" persistent>
    <q-card style="background-color: var(--bg-color); color: var(--text-color)">
      ...
      <q-card-section class="q-pt-none">
        <q-input dense :label="t('admin.credentialMapping.categoryName')" v-model="catname" autofocus
                 @keyup.enter="prompt = false" :placeholder="t('common.category')">
          <template v-slot:append>
            <q-icon name="edit"/>
          </template>
      ...
      <q-card-actions align="right" class="text-primary">
        <q-btn flat v-close-popup> {{ t("common.cancel") }}</q-btn>
        <q-btn flat v-close-popup @click="updateCategory(editedRow.id)" :disable="!catname"> {{ t("common.save") }}
        </q-btn>
      </q-card-actions>
    </q-card>
  </q-dialog>
...
export default {
  ...
  setup() {
    ...
    const columns = [
      {
        name: 'category',
        required: true,
        label: t('admin.credentialMapping.categoryName'),
        align: 'center',
        field: row => row.name,
        sortable: true
      },
      {
        ...
        align: 'center',
        label: t('admin.credentialMapping.internal'),
        field: row => row.credentials,
        format: (val) => val.map(c => c.name).join(', '),
        sortable: true
      },
      {
        ...
        align: 'center',
        label: t('admin.credentialMapping.external'),
        field: row => row.externalCredentials,
        format: (val) => val.map(e => e.name).join(', '),
        sortable: true
      },
    ...
    credentialStore.getExternalCredentials()

    credentialStore.getCredentials()

    function deleteCategory(id) {
      credentialStore.deleteCategory(id).then(() => {
        credentialStore.getCategory().then((categories) => {
          rows.value = categories
        })
      })
    ...
    function createCategory() {
      credentialStore.createCategory(address.value, model.value, model2.value).then(() => {
        credentialStore.getCategory().then((categories) => {
          rows.value = categories
        })
        address.value = ''
    ...
    function updateCategory(id) {
      credentialStore.updateCredentials(id, catname.value, model3.value, model4.value).then(() => {
        credentialStore.getCategory().then((categories) => {
          rows.value = categories
        })
      })
