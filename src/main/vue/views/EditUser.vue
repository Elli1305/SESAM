    ...
    <q-page class="column justify-evenly" style="padding: 2em 5em" v-if="user!=null">
        ...
        <div class="column justify-evenly self-center no-wrap" style="width: 80vw; height: 50vh">
            ...
            <div class="row justify-around">
                <q-btn round color="negative" text-color="positive" icon="delete" @click="deleteAlert = true"
                       style="width: 4em; height: 4em"/>
                <q-btn round color="positive" text-color="negative" @click="saveEditedUser()" icon="save"
                       style="width: 4em; height: 4em"/>
            </div>
        </div>
    </q-page>
...
export default {
    ...
    setup(props) {
        ...
        userStore.getEditUser(props.email).then(() => {
            ...
            if (adminRole) {
                roles.value.push(t('profile.administrator'));
            }
            if (editorRole) {
                roles.value.push(t('profile.editor'));
            }
            if (issuerRole) {
                roles.value.push(t('profile.issuer'));
            }
        ...
        function saveEditedUser() {
            ...
            userStore.saveEdits(prename, lastname, mail, roles.value.map(r => {
                if (r === t('profile.administrator')) {
                    return 'ADMINISTRATOR'
                }
                if (r === t('profile.editor')) {
                    return 'EDITOR'
                }
                if (r === t('profile.issuer')) {
                    return 'ISSUER'
                }
            })).then(() => {
        ...
        function deleteUser() {
            ...
            if (mail === currentUser.username) {
                ...
                userStore.deleteUser(mail).then(() => {
                    axios.get('/api/user')
                    router.push('/currentuserlist')
                });
        ...
        return {
            ...
            options: [
                t('profile.administrator'), t('profile.editor'), t('profile.issuer')
            ]
        }
    }
}
